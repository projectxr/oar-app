...
use llm_cpp::{llama_grammar, llama_grammar_element, llama_gretype};
use std::ptr::NonNull;
use std::str::FromStr;
use tracing::error;
...
impl Clone for LlamaGrammar {
    fn clone(&self) -> Self {
        let grammar = unsafe { llm_cpp::llama_grammar_copy(self.grammar.as_ptr()) };
        Self {
    ...
    fn parse_alternatives<'a>(
        ...
        while rest.starts_with('|') {
            rule.push(llama_grammar_element {
                type_: llm_cpp::LLAMA_GRETYPE_ALT,
                value: 0,
            });
        ...
        rule.push(llama_grammar_element {
            type_: llm_cpp::LLAMA_GRETYPE_END,
            value: 0,
        });
        ...
        while !rest.is_empty() {
            ...
            if first_char == '"' {
                ...
                while !rest.starts_with('"') {
                    ...
                    rest = r;
                    rule.push(llama_grammar_element {
                        type_: llm_cpp::LLAMA_GRETYPE_CHAR,
                        value: c as _,
                    });
                }
                ...
                let start_type = if rest.starts_with('^') {
                    rest = &rest[1..];
                    llm_cpp::LLAMA_GRETYPE_CHAR_NOT
                } else {
                    llm_cpp::LLAMA_GRETYPE_CHAR
                };
                last_sym_start = rule.len();
                while !rest.starts_with(']') {
                    let (c, r) = Self::parse_char(rest)?;
                    ...
                    let gre_type = if last_sym_start < rule.len() {
                        llm_cpp::LLAMA_GRETYPE_CHAR_ALT
                    } else {
                        start_type
                    };
                    rule.push(llama_grammar_element {
                    ...
                    if rest.starts_with("-]") {
                        ...
                        rest = r;
                        rule.push(llama_grammar_element {
                            type_: llm_cpp::LLAMA_GRETYPE_CHAR_RNG_UPPER,
                            value: c as _,
                        });
                    }
                }
                ...
                last_sym_start = rule.len();
                rule.push(llama_grammar_element {
                    type_: llm_cpp::LLAMA_GRETYPE_RULE_REF,
                    value: ref_rule_id,
                });
            } else if first_char == '(' {
...
impl FromStr for LlamaGrammar {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let grammar =
            unsafe { llm_cpp::llama_grammar_init(rules, n_rules, root_id as usize) };
...
impl Drop for LlamaGrammar {
    fn drop(&mut self) {
        unsafe { llm_cpp::llama_grammar_free(self.grammar.as_ptr()) }
    }
}
